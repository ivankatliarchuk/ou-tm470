...
func loadBlockchain() error {
	...
	log.Printf("INFO: serviceChain.loadBlockchain(): Loaded Blockchain, total records = %s", string(len(Blockchain)))
...
func replaceChain(newBlock Block) bool {
	...
	if isBlockValid(newBlock, Blockchain[len(Blockchain)-1]) {
		...
		if err != nil {
			log.Printf("ERROR: Unable to write blockchain to disk: %s", string("md5589_blockchain_"+strconv.Itoa(BlockchainLength)))
		}
		return true
	}
	return false
